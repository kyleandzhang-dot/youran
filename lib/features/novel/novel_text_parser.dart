...
class NovelTextParser {
  const NovelTextParser({
    // 只增加单页可显示字数，不改变原来的换行和阅读节奏。
    this.maxCharsPerPage = 190,
    this.mergeNarrationMaxChars = 150,
  });
  ...
  List<String> _completeTailPieces(String chunk) {
    ...
    for (var i = 0; i < chunk.length; i++) {
      if ('。！？…?!'.contains(chunk[i])) {
        final piece = chunk.substring(start, i + 1).trim();
        if (piece.isNotEmpty) result.add(piece);
        start = i + 1;
      }
    }
    if (start >= chunk.length) return result;
    return result;
  }
  ...
  List<String> _splitLongChunk(String chunk) {
    ...
    if (body.trim().length <= maxCharsPerPage) return <String>[chunk.trim()];
    ...
    for (final token in tokens) {
      if (current.isNotEmpty && current.length + token.length > maxCharsPerPage) {
        result.add('$prefix$current'.trim());
        current = token.trimLeft();
      } else {
        current = current.isEmpty ? token.trimLeft() : '$current$token';
      }
    }
    if (current.trim().isNotEmpty) result.add('$prefix$current'.trim());
    return result;
  }
  ...
  List<NovelSentence> buildSentences({
    ...
    if (message.sentenceItems.isNotEmpty) {
      return mergeNarrations(message.sentenceItems);
    }
